...
import { Star, CheckCircle, XCircle, Clock, User, TrendingUp } from 'lucide-react';
...
export default function Reviews() {
  ...
  const stats = {
    ...
    npsPromedio: reviews.length > 0
      ? Math.round((reviews.reduce((sum, r) => sum + r.score_nps, 0) / reviews.length) * 10) / 10
      : 0
  };
    ...
    <DashboardLayout>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        <Card>
          <div className="p-6">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-gray-600">NPS Promedio</span>
              <TrendingUp className="w-5 h-5 text-purple-600" />
            </div>
            <div className="text-3xl font-bold text-purple-600">{stats.npsPromedio}</div>
          </div>
        </Card>
